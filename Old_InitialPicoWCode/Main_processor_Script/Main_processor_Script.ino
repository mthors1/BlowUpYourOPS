...
/* ───────────────────── PIN & STATE SETUP ────────────────────── */
const int segmentPins[] = {15, 14, 1, 0, 11, 10, 9, 8};   // A‑G, DP
const int digitPins[]   = {7, 6, 16, 17};                 // digits 1‑4
const int buzzerPin     = 18;
const int buttonPin     = 26;

const int ledPins[]     = {19, 20, 21, 22};               // BL‑R, TR‑R, BR‑Y, TL‑Y
...
/* forward declares */
int  getDigitValue(int d);
void showDigit(int d,int num);
void clearDisplay();
...
void setup() {
  ...
  Serial1.setTX(12); Serial1.setRX(13); Serial1.begin(115200);
  Serial2.setTX(4);  Serial2.setRX(5);  Serial2.begin(115200);

  /* non‑blocking handshake: wait max 2 s for any byte */
  unsigned long t0 = millis();
  while (millis() - t0 < 2000) {
    if (Serial1.available() || Serial2.available()) break;
    Serial1.write((uint8_t)0);
    Serial2.write((uint8_t)0);
    delay(50);
  }
...
void loop() {
  ...
  /* idle until start button pressed */
  if (digitalRead(buttonPin) == HIGH && !initialized) {
    Serial1.write((uint8_t)0);
    Serial2.write((uint8_t)0);
    return;                             // keep looping
  }

  if (!initialized) {                     // first pass after press
    initialized  = true;
    finalBuzzed  = false;
    totalSeconds = 5 * 60;
  }

  /* ---------- poll SERIAL2 (wire / maze slave) ---------- */
  if (Serial2.available()) {
    uint8_t s2 = Serial2.read();
    Serial2.write((uint8_t)1);            // heartbeat

    if (!wireGameDone) {                  // wire‑cut phase
      if (s2 == 3)                        // wrong wire
        totalSeconds = max(0, totalSeconds - 30);
      else if (s2 == 4) {                 // wire game solved
        wireGameDone = true;
        Serial1.write((uint8_t)1);        // start memory / RGB slave
      }
    } else {                              // maze / code phase
        if (memory_rgb_done){
        /* handle maze bytes 1‑9 here exactly as before … */
          Serial2.write((uint8_t)6);
          if (s2 == 7){
  ...
  /* ---------- poll SERIAL1 (memory / RGB slave) ---------- */
  if (Serial1.available()) {
    uint8_t s1 = Serial1.read();

    if (!memory_rgb_done) {
      if (s1 == 4 && !memory_done) memory_done = true;
      if (s1 == 5 &&  memory_done) memory_rgb_done = true;
    }
  }

  /* -------- countdown / display / flash logic ----------- */
  static unsigned long lastUpdate=0, lastBeep=0, lastFlash=0;
  static bool ledPhase=false;

  // multiplex 4‑digit 7‑segment
  for (int d=0; d<4; d++){ showDigit(d,getDigitValue(d)); delay(2); clearDisplay(); }
  ...
  if (millis() - lastUpdate >= 1000) {
    ...
    if (totalSeconds > 0) totalSeconds--;
      ...
      /* explosion sequence unchanged (shortened here) */
      initialized=false; finalBuzzed=true;
      for (int i=0;i<4;i++) digitalWrite(ledPins[i],LOW);
    }
  }

  // pacing beep & LED flash
  if (totalSeconds > 0) {
    unsigned long now=millis();
    int interval, pitch=700;
    if (totalSeconds>30) interval=1000;
    else if (totalSeconds>10) interval=map(totalSeconds,30,10,1000,250);
    else { interval=map(totalSeconds,10,0,250,80); pitch=800+(10-totalSeconds)*10; }

    if (now-lastBeep>=interval) { lastBeep=now; tone(buzzerPin,pitch,min(120,interval-10)); }
    if (now-lastFlash>=interval) {
      lastFlash=now; ledPhase=!ledPhase;
      digitalWrite(22,ledPhase); digitalWrite(21,ledPhase);
      digitalWrite(20,!ledPhase);digitalWrite(19,!ledPhase);
    }
  }
...
int getDigitValue(int d){
  ...
  if (d==0) return m/10; if (d==1) return m%10;
  if (d==2) return s/10; return s%10;
}
void showDigit(int d,int num){
  digitalWrite(digitPins[d],LOW);
  byte seg=digits[num];
  for(int i=0;i<7;i++) digitalWrite(segmentPins[i],(seg>>i)&1);
  digitalWrite(segmentPins[7], (d==1)?HIGH:LOW);
}
void clearDisplay(){
  for(int i=0;i<4;i++) digitalWrite(digitPins[i],HIGH);
  for(int i=0;i<8;i++) digitalWrite(segmentPins[i],LOW);
